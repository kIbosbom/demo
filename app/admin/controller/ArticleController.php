...
class ArticleController extends AdminCheckAuth
{
    ...
    public function getContentList()
    {
        ...
        $content = \think\loader::model('Article')
                ->field('*,w.id as cate_id,a.id as id,w.title as cate_title,a.title as title,a.create_time')
                ->alias('a')
    ...
    public function c()
    {
        if($this->request->isPost()){
            $param = vae_get_param();
			$param['create_time'] = strtotime($param['create_time']);
            Db::name('article')->where('type_c',1)->update($param);
            return vae_assign();
        }else{
            $data = Db::name('article')->where('type_c',1)->find();
